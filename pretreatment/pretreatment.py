...
def eachFile(keyword,filepath,toFilepath):
	global FilePath
	...
	for file in files:
		...
		cutcontent(keyword,filepath,file,toFilepath)

def cutcontent(keyword,filepath,filename,toFilepath):
	...
	newFile.close()
	select(keyword,newFileName,item.date,toFilepath)
	# os.remove(newFileName)

def select(keyword,filename,date,toFilepath):
	global FilePath
	...
	lastfile = open(toFilepath+str(date)[:4]+'.txt','a')
	lastfile_without = open(toFilepath+str(date)[:4]+'_without.txt','a')
	print len(items)
	for item in items:
		# print item
		if keyword in item:
...
if __name__ == "__main__":
	# to final 2010.txt
	keyword			= "機関"
	toFilepath   = "/home/dreamer/codes/my_code/code/download/pretreatment_data/"
	fromFilepath = "/home/dreamer/codes/my_code/code/download/database/"
	eachFile(keyword,fromFilepath, toFilepath)
